...
| `version` | `0.1.0` | 语义化版本。公共 API 或数据格式变更时同步更新。 |
...
`LanguageDataLoader` 会尝试以下 Addressables 地址：
...
WebGL Player 不支持同步 `WaitForCompletion`。WebGL 首次加载语言数据时应使用异步加载策略，避免直接调用当前同步查询流程。
