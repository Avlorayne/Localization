...
3. 选中导入后的 `Resources/Localization/LanguageConfig.asset`，将 `sourceFolderPath` 指向第 2 步的目录；将 `soFolderPath` 保持为 `Assets/Resources/Localization` 或项目约定目录。
...
配置文件默认从 `Resources/Localization/LanguageConfig.asset` 加载，对应运行时路径为 `Localization/LanguageConfig`。
